	 ...
	 function insert(){		
		...
		if($link){				
			foreach($spiderarr as $i=>$thepost){	
				...
				$titlesql="select * from jz_bjpost where title="."'".$thetitle."'";
				mysql_query("SET NAMES UTF-8");   
				$titleresult=mysql_query($titlesql,$link);
				$titleinfo=mysql_fetch_array($titleresult);				
				if(!$titleinfo){	
					$numsql="select count(*) from jz_bjpost";
					...
					$sql = "INSERT INTO jz_bjpost(id,postId,username,name,title,area,payTypeId,salary,salaryTypeId,numwant,detail,time,phone,weixin,qq)VALUES('',$num,'{$username}','{$name}','{$title}','0','0','','0','','{$detail}','{$newtime}','{$phone}','{$weixin}','{$qq}')";					
