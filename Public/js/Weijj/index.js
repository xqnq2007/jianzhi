$(function(){
	...
	$("#mainlist").on('click','.comtSendBtn',function(){		
		...
		if($.trim(postcont)){
			$.post("/tj/Weijj/postComt",{"postid":postid,"postcont" :postcont },function(data){				
					var html = "";					
					if(data['status']=='1'){							
	...
	$(window).scroll(function(){		
		...
		if($(document).scrollTop() > 
			...
			$.get("/Weijj/getDbMore",{"last_id" : $("#mainlist>div:last>input").val()},function(data){	
					var html = "";					
					if($.isArray(data)){						
						for(i in data){							
							...
							html+="</li><li class=\"list-group-item comtBtnLi h28\" >"+
								"<span class=\"time\">"+data[i]['time']+"</span>"+"<span>"+	
